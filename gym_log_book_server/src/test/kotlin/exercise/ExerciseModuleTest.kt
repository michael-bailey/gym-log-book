...
import net.michael_bailey.gym_log_book.server.exercise.ExerciseModule
...
class ExerciseModuleTest {
	...
	private val exerciseModule = ExerciseModule()

	@Test
	fun `coroutineScope returns an active scope`() {
		val coroutineScope = exerciseModule.coroutineScope()
	...
	fun `coroutineScope creates a new scope on each direct call`() {
		val firstScope = exerciseModule.coroutineScope()
		val secondScope = exerciseModule.coroutineScope()
